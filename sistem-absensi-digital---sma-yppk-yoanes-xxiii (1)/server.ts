...
// Upload Logo Endpoint (Persists to server disk so every phone & PC sees it)
app.post('/api/upload-logo', (req, res) => {
  try {
    const { type, dataUrl } = req.body; // type: 'kiri' | 'kanan'
    if (!type || !dataUrl) {
...
app.post('/api/send-wa', async (req, res) => {
  try {
    ...
    if (apiToken && currentSettings.aktifkan_wa_api) {
      // Call Fonnte / WhatsApp Gateway API
    ...
    // If no active paid token, log the simulated dispatch and return success for client UI
